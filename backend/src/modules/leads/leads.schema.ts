...
const body = z.object({
  ...
  sourceId: z.string().min(1).max(36).nullable().optional(),
  status: z
    .enum(["NEW", "CONTACTED", "QUALIFIED", "UNQUALIFIED", "LOST", "CONVERTED", "DEMO"])
    .optional(),
...
export const assignSchema = z.object({
  body: z.object({ userId: z.string().min(1).max(36).nullable() }),
  query: z.any(),
  params,
});
export const statusSchema = z.object({
  body: z.object({
    status: z.enum(["NEW", "CONTACTED", "QUALIFIED", "UNQUALIFIED", "LOST", "DEMO"]),
  }),
  query: z.any(),
  params,
});
